...
```
고스톱 프로그램 만들기 
- 같은 카드 세장이 들어왔을 때 흔들어 점수를 두 배로 만드는 규칙은 사용자의 선택이 필요해 복잡하고 한 번에 하나의 카드를 내는 규칙에 예외가 생겨 제외한다.
- 두 장의 피로 계산되는 쌍피, 피와 십짜리 양쪽으로 쓸 수 있는 카드, 보너스 카드 등은 점수 계산이 복잡해서 제외한다.
- 피박, 광박 등 점수를 곱절로 만드는 규칙도 제외한다. 어차피 현금이 오갈 수 없고 수동으로 적용 가능하므로 점수를 평이하게 매긴다.
```
...
void OutPrompt(const char* Mes, int Wait/*o*/)
{
	gotoxy(5, 23);
...
int InputInt(const char* Mes, int start, int end)
{
	...
	for (;;) {
		...
		OutPrompt("무효한 번호입니다. 지정한 범위에 맞게 다시 입력해 주세요", 1000);
	}
}
// 프로그램을 총지휘하는 main 함수 
int main()
{
	...
	for (SouthTurn = true; !Deck.IsEmpty(); SouthTurn = !SouthTurn) {
		...
		if (ch == 0) {
			if (InputInt("정말 끝내실 겁니까?(0:예, 1:아니오)", 0, 1) == 0)
				return 0;
			else
				continue;
		}
		...
		if (Deck.IsNotLast()) {
			...
			if (UserTriple || DeckTriple) {
				OutPrompt("한꺼번에 세장을 먹었습니다.", PromptSpeed);
